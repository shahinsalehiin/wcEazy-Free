...
function wceazy_pdf_invoice_packing_slips_init() {
    // init module
    $('#wceazy_pdf_invoice_header').show();
    $('#wceazy_pdf_invoice_container').show();
};

var $wfPdfInvoiceAdmin = {

    wfpi_settings       : JSON.parse(wceazy_pi_admin_object.wceazy_pi_settings),
    wfpi_settings_form  : "#wceazy_form_pdf_invoice",
    wfpi_file_uploader  : ".wceazy_file_uploader",
    ...
    init_wfpdf_invoice_admin: function(){
        ...
        jQuery(document).ready(function($) {
            ...
            $(".wceazy_pdf_invoice_right_content .wceazy_pdf_invoice_tab_body").hide();
            $(".wceazy_pdf_invoice_right_content .wceazy_pdf_invoice_tab_body[data-id='wceazy_pdf_invoice_document']").show();

            $(".wceazy_pdf_invoice_data_tabs .tab_item").unbind("click");
            $(".wceazy_pdf_invoice_data_tabs .tab_item").bind("click", function (e) {
                $(".wceazy_pdf_invoice_data_tabs .tab_item").removeClass('tab_item_active');
                $(this).addClass('tab_item_active');

                $(".wceazy_pdf_invoice_right_content .wceazy_pdf_invoice_tab_body").hide();
                $(".wceazy_pdf_invoice_right_content .wceazy_pdf_invoice_tab_body[data-id='" + $(this).data('target') + "']").show();
            });

            //file uploader
            $wfPdfInvoiceAdmin.wceazy_pdf_invoice_media_upload();
            ...
            //wceazy customize invoice preview
            $wfPdfInvoiceAdmin.wceazy_customize_invoice_preview();

            // trigger customize invoice controls
            $wfPdfInvoiceAdmin.wceazy_customize_invoice_controls();

            // trigger customize shipping slip controls
            $wfPdfInvoiceAdmin.wceazy_customize_shipping_slip_controls();

            // trigger customize shipping slip
            $wfPdfInvoiceAdmin.wceazy_customize_shipping_slip_preview();
    ...
    // Media Upload
    wceazy_pdf_invoice_media_upload : function () {
    ...
    // define wceazy customize invoice preview
    wceazy_customize_invoice_preview : function(){
    ...
    wceazy_customize_invoice_controls : function(){
    ...
    wceazy_customize_shipping_slip_controls : function(){
    ...
    // define wceazy customize shipping slip preview
    wceazy_customize_shipping_slip_preview : function(){
    ...
    submit_wfpi_settings : function (e) {
        e.preventDefault();
        let $submit_button = $('.wceazy_pdf_invoice_btn');
        $submit_button.text( 'Please Wait....' );
        $submit_button.addClass( 'wceazy-pi-btn-disabled' );
        $submit_button.prop( 'disabled', true );

        let $post_data = {'action': 'wceazy_save_pdf_invoice_settings', 'data': $($wfPdfInvoiceAdmin.wfpi_settings_form).serializeArray()};
        ...
        $.ajax({
            ...
            success: function(res){
                ...
                if ($obj.status == 'true') {
                    ...
                    $submit_button.removeClass('wceazy-pi-btn-disabled');
