...
'use strict';
function wceazy_auto_apply_coupon_init(){
    $('#wceazy_auto_apply_coupon_header').show();
    $('#wceazy_auto_apply_coupon_container').show();
    init_wceazy_auto_coupon_data_table();
};
...
jQuery(document).ready(function ($) {
    ...
    $('#wfaac_per_page_view').on('change', function () {
        ...
        init_wceazy_auto_coupon_data_table($pageLength);
    });

    // Control Bulk Checkbox
    $('#wceazy_auto_apply_coupon_table .wfaca_bulk_check').on('change', function (e) {
        if (this.checked) {
            var totalChecked = 0;
    ...
    let $wfaabc_toggole_scope = $('#wceazy_auto_apply_coupon_container').find('.btn-wfaabc-toggle').length;
    ...
    // for keyword search
    let $wfaabc_ks_scope = jQuery('#wceazy_auto_apply_coupon_container').find('.wfaac-keword-search').length;
    if($wfaabc_ks_scope > 0){
        $('.wfaac-keword-search').on('keyup', function () {
            wceazy_auto_apply_coupon_keywordsearch();
        });
    }else{
        console.log('Oops: scope not found, please try later!');
    }
...
function bulk_auto_apply_coupon_toggle() {
    ...
    let $selectedItems = [];
    jQuery('.auto_apply_coupon_popup').addClass('wceazy_popup_open');
    jQuery('.auto_apply_coupon_popup .wceazy-popup-content').text('Do you want to '+$wfaabc_toggole_type+' selected coupons to Auto Apply list ?');
    ...
    if( $selectedItems.length > 0 ){
        ...
        $('.wceazy-btn-trigger-success').on('click', function () {
            let $post_data = {'action': 'wceazy_auto_apply_coupon_toggle', 'data': $couponData};
            $.ajax({
                url: ajaxurl, type: "POST", data: $post_data,
                success: function (data) {
                    var obj = JSON.parse(data);
                    if (obj.status == 'true') {
                        jQuery('.auto_apply_coupon_popup').removeClass('wceazy_popup_open');
...
function wfaca_close_popup() {
    $('.auto_apply_coupon_popup').removeClass('wceazy_popup_open');
}

function add_to_auto_apply_coupon(couponData) {
    jQuery('.auto_apply_coupon_popup').addClass('wceazy_popup_open');
    var couponCode = couponData.coupon_code;
    jQuery('.auto_apply_coupon_popup .wceazy-popup-content').text('Do you want to add this coupon('+couponCode+') to Auto Apply List ?');
    let $post_data = {'action': 'wceazy_auto_apply_coupon_add', 'data': couponData};
    // trigger action
    $('.wceazy-btn-trigger-success').on('click', function () {
        $.ajax({
            url: ajaxurl, type: "POST", data: $post_data,
...
function remove_auto_apply_coupon(couponData) {
    jQuery('.auto_apply_coupon_popup').addClass('wceazy_popup_open');
    var couponCode = couponData.coupon_code;
    jQuery('.auto_apply_coupon_popup .wceazy-popup-content').text('Do you want to remove this coupon('+couponCode+') to Auto Apply List ?');
    let $post_data = {'action': 'wceazy_auto_apply_coupon_remove', 'data': couponData};
    // trigger action
    $('.wceazy-btn-trigger-success').on('click', function () {
        $.ajax({
            url: ajaxurl, type: "POST", data: $post_data,
...
function wceazy_auto_apply_coupon_check_items() {
...
function wf_auto_apply_coupon_check_items_control() {
    let $check_items = wceazy_auto_apply_coupon_check_items();
    if ($check_items) {
        jQuery("#wceazy_auto_apply_coupon_container .btn-wfaabc-toggle").removeClass('btn-disabled');
        jQuery("#wceazy_auto_apply_coupon_container .btn-wfaabc-toggle").prop('disabled', false);
    } else {
        jQuery("#wceazy_auto_apply_coupon_container .btn-wfaabc-toggle").addClass('btn-disabled');
        jQuery("#wceazy_auto_apply_coupon_container .btn-wfaabc-toggle").prop('disabled', true);
    }
}

// define init auto coupon data table
function init_wceazy_auto_coupon_data_table($pageLength=10) {
    ...
    var $data = {};
    let $post_data = {'action': 'wceazy_auto_apply_coupon_list', 'data': $data};
    var table = jQuery('#wceazy_auto_apply_coupon_table').DataTable({
        processing: true,
        serverSide: false,
...
function wceazy_auto_apply_coupon_keywordsearch() {
    // Declare variables
    ...
    $table = document.getElementById("wceazy_auto_apply_coupon_table");
