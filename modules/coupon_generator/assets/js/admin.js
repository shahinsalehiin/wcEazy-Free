...
function wceazy_coupon_generator_init() {
    // init module
    $('#wceazy_coupon_generator_header').show();
    $('#wceazy_coupon_generator_container').show();

    $("#wceazy_coupon_generate_dashboard .coupon_tab_body").hide();
    $("#wceazy_coupon_generate_dashboard .coupon_tab_body[data-id='coupon_tab_general']").show();

    $("#wceazy_coupon_generate_dashboard .coupon_data_tabs .tab_item").unbind("click");
    $("#wceazy_coupon_generate_dashboard .coupon_data_tabs .tab_item").bind("click", function () {

        $("#wceazy_coupon_generate_dashboard .coupon_data_tabs .tab_item").removeClass('tab_item_active');
        $(this).addClass('tab_item_active');

        $("#wceazy_coupon_generate_dashboard .coupon_tab_body").hide();
        $("#wceazy_coupon_generate_dashboard .coupon_tab_body[data-id='" + $(this).data('target') + "']").show();
    });

    // for close poopup
    $('.wceazy_close_popup').on('click', function () {
        $('.wceazy_popup').removeClass('wceazy_popup_open');
    });
...
function wceazy_submit_coupon_generator() {
    ...
    $('.wceazy_popup').addClass('wceazy_popup_open');
    ...
    let $post_data = {'action': 'wceazy_coupon_generator_init', 'data': $postData};
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (obj.status == 'true') {
                ...
                $('#form_wceazy_coupon_generator')[0].reset();
                $('#form_wceazy_coupon_generator .select2-selection__rendered').val("").text("Please select").trigger('change');
