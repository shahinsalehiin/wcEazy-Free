...
function wceazy_product_sticky_bar_init(){
    $('#wceazy_product_sticky_bar_header').show();
    $('#wceazy_product_sticky_bar_container').show();
};

/*-- for submit product sticky bar settings --*/
function wceazy_submit_product_sticky_bar() {
    ...
    let $post_data = {'action': 'wceazy_product_sticky_bar_settings', 'data': $postData};
...
jQuery(document).ready(function () {
    ...
    var $wceazy_psb_settings = JSON.parse(wceazy_psb_jsadmin.wceazy_psb_settings);
    var $psb_settings = Object.keys($wceazy_psb_settings).length;
    if( $psb_settings <= 0 ){
