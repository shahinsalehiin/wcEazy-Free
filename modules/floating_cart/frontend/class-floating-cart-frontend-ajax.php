...
if (!class_exists ('wcEazyFloatingCartFrontendAjax')) {
    class wcEazyFloatingCartFrontendAjax
    {
        ...
        public function __construct ($base_admin)
        {
            ...
            $this->wfc_settings = get_option('wceazy_floating_cart_settings', false);

            add_action ('wp_ajax_wceazy_init_floating_cart_content', array($this, 'wceazy_init_floating_cart_content'));
            add_action( 'wp_ajax_nopriv_wceazy_init_floating_cart_content', array($this, 'wceazy_init_floating_cart_content') );

            add_action ('wp_ajax_wceazy_floating_cart_update', array($this, 'wceazy_floating_cart_update'));
            add_action( 'wp_ajax_nopriv_wceazy_floating_cart_update', array($this, 'wceazy_floating_cart_update') );
            ...
            add_action ('wc_ajax_wceazy_floating_cart_add_item', array($this, 'wceazy_floating_cart_add_item'));

        }

        public function wceazy_init_floating_cart_content(){

            include_once WCEAZY_MODULES_PATH . "/" . $this->base_admin->module_slug . "/frontend/api/init-floating-cart-content.php";
            wp_die ();
        }

        public function wceazy_floating_cart_update ()
        {
            include_once WCEAZY_MODULES_PATH . "/" . $this->base_admin->module_slug . "/frontend/api/floating-cart-update.php";
            wp_die ();
        }

        public function wceazy_floating_cart_apply_coupon ()
        {
            include_once WCEAZY_MODULES_PATH . "/" . $this->base_admin->module_slug . "/frontend/api/apply-coupon.php";
            wp_die ();
        }
            ...
            if(!empty($data)){
                ...
                if( !empty($cart_item) ){
                    ...
                    if( $updated ){
                        ...
                        $notice_type = 'error';
                        $notice = 'Opps: something is wrong!';
                    }
         ...
         * Develop By : Sm. Sazzad
         **/
        public function wceazy_add_coupon_in_cart ($data = array()){
            ...
            if (!empty($data) && !empty($data['coupon_code'])) {
                ...
                if (!is_wp_error ($valid_response)) {
                    ...
                    if (!$woocommerce->cart->has_discount (sanitize_text_field ($coupon_code))) {
                        ...
                        $notice_type    = 'success';
                        $notice         = __ ('Coupon applied successfully!!', 'wceazy');
                    } else {
                        $notice_type    = 'error';
        ...
        public function wceazy_floating_cart_add_item(){
            include_once WCEAZY_MODULES_PATH . "/" . $this->base_admin->module_slug . "/frontend/api/floating-cart-refresh.php";
            wp_die ();
        }
